 ...
 * A01023656 - A01023785
 * Programa: solución fase 3/6 RETO
...
{
    public:
        list<string> *nombresNoReto;
        int size;

        Set(int s)
        {
            size = s;
            nombresNoReto = new list<string>[size];
        }

        int hashPlegamiento(string nombre)
        {
            string a = nombre;
            int ascChar;
            int suma = 0;
        ...
        void print()
        {
            ...
            {
                cout<<"[" << i << "]: ";
                for(auto j = nombresNoReto[i].begin(); j != nombresNoReto[i].end(); j++)
                {
                    cout << *j << ",";
                }
                cout<<endl;
...
int main()
{
    ...
    {
        a = conexiones[j].nombreFuente;

        if (a.find("reto.com") == string::npos)
        {
            pos = s.hashPlegamiento(conexiones[j].nombreFuente);
            //cout << pos << endl;
            s.nombresNoReto[pos].push_back(conexiones[j].nombreFuente);
        }

        a = conexiones[j].nombreDestino;
        if (a.find("reto.com") == string::npos)
        {
            pos = s.hashPlegamiento(conexiones[j].nombreDestino);
            s.nombresNoReto[pos].push_back(conexiones[j].nombreDestino);
            //s.nombresNoReto->push_back(a);
        }
    }s.print();
    return 0;
}
